...
func (d *sshfsDriver) mountVolume(name, destination string) error {
	...
	cmd := fmt.Sprintf("sshfs -o IdentityFile=/root/.ssh/id_rsa -o allow_other -o default_permissions -o follow_symlinks -o transform_symlinks -o sshfs_debug %s %s", name, destination)
